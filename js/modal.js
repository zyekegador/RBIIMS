...
function openModal(id) {
  ...
  setTimeout(function () {
    var f = el.querySelector(
      "input:not([readonly]):not([type=checkbox]), select",
    );
    if (f) f.focus();
  }, 230);
}
...
function _clearForm() {
  ...
  ["f-gender", "f-citizenship", "f-civil", "f-purok"].forEach(function (id) {
    _sv(id, "");
  });
  _sv("f-role", "Member");
...
function openAddModal() {
  ...
  if (typeof currentPurok !== "undefined") {
    ...
    var maxHH = existing.reduce(function (m, r) {
      return Math.max(m, Number(r.household) || 0);
    }, 0);
...
function openEditModal(id) {
  ...
  // Indicators
  _sv("f-employmentType", r.employmentType || "Unemployed");
...
function saveResident() {
  ...
  var payload = {
    ...
    employmentType: _fv("f-employmentType"),
    seniorCitizen: isSC,
    ofw: _fc("f-ofw"),
    pwd: _fc("f-pwd"),
